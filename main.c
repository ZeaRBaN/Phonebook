...
void multi()
{
    ...
    for(i=0; i<count; i++)
    {
        if ( strcasecmp(fname,"")!= 0 || strcasecmp(fname,p[i].fname) != 0 )
            flag=0;
        if ( strcasecmp(lname,"")!= 0 || strcasecmp(lname,p[i].lname) != 0 )
            flag=0;
        if ( strcasecmp(number,"")!= 0 || strcasecmp(number,p[i].number) != 0 )
            flag=0;
        if ( strcasecmp(mail,"")!= 0 || strcasecmp(mail,p[i].mail) != 0 )
            flag=0;
        if (flag == 1)
        {
        printf("Name:%s %s\n",p[i].fname, p[i].lname);
